...
<script type='text/javascript' src='http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js'></script>";
  ...
  $DBdates=mysql_query("SELECT DISTINCT `date` FROM `zakaz` WHERE `show`=1 ORDER BY `date`;");
  $Nd=0;
...
<select id="date_s" class=input size=1 onchange="ShowDiv(<?=$Nd; ?>,0)">
  <option value='0'>== Новые записи ==</option>
<?
for ($i=1;$i<=$Nd;$i++)
  {  
  $str = $dates[$i];
...
echo "<div id='0' style=\"display: block;\">\n";
  ...
  while ($zak = mysql_fetch_array($tickets))
	...
	if ((!$zak['sent']) && (''!=$zak['email']))  
	  { echo "      <a href='mail.php?id={$zak['id_zak']}&key=".md5($zak['id_zak'].$zak['name'])."'><img src=mail.png></a>\n"; }
	else
	  { if (($zak['sent']) && (''!=$zak['email'])) { echo "<img src=mailed.png>"; }
		...
		if ((!$zak['sent']) && (''==$zak['email']))  
	      {
		  echo "      <a href='mail.php?id={$zak['id_zak']}&key=".md5($zak['id_zak'].$zak['name'])."&ok=true'><img src=OK.png></a>\n";
		  }
		else
		  {
    ...
    echo "	  <a href='aus.php?id={$zak['id_zak']}&key=".md5($zak['id_zak'].$zak['name'])."'><img src=delete.png></a>
	</td>
  </tr>\n";
    };
...
<br><br>
<a href='aus.php?do=hide_date&date=<?=$dates[$table];?>' class=link>Скрыть заказы на этот спектакль</a>
</div>
<?
  };
?>
...
<br>
<a href='tickets_old.php' class=link>Архив заказов</a>
<?
